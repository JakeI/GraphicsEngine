...
GeneticRootScene::GeneticRootScene() : RootScene()
{
	...
	tsCenter = new TestScene(this, glm::vec4(0.0f, 1.0f, 1.0f, 1.0f), 0.1f, (float)M_PI / 4000.0f);
	add(tsCenter);
	...
	button = new Button(this, "res/atlases/EllipseButton");
	add(button);
	verticalLayout->add(button, WeightedLayout::Info(80));

	tsRed = new TestScene(this, glm::vec4(1.0f, 0.0f, 0.0f, 1.0f),
		(float)M_PI/3.0f, (float)M_PI / 10240.0f);
	add(tsRed);
	verticalLayout->add(tsRed, WeightedLayout::Info(100));

	tsBlue = new TestScene(this, glm::vec4(0.0f, 0.0f, 1.0f, 1.0f),
		(float)M_PI / 6.0f, -(float)M_PI / 5120.0f);
	add(tsBlue);
...
void GeneticRootScene::increment(float time, float deltaTime)
{
	...
	static const float omega = 2 * (float)M_PI / 1000.0f;
	static const float phi = 2 * (float)M_PI / 2000.0f;
